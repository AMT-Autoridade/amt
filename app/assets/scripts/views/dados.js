...
import { rawGitApi, startYear, endYear } from '../config';
    ...
    return (
      <div id='dados' className='container-wrapper'>
        <section id='dados' className='content-wrapper'>
          ...
          <div>
            <p className='lead'> Este site contém dados sobre o número de <a href={`${rawGitApi}@master/data/taxis.csv`} title='Descarregar dados'>táxis licenciados e os contingentes estabelecidos</a> (número máximo de táxis a licenciar) pelos municípios. São também utilizados dados relativos ao <a href={`${rawGitApi}@master/data/area-metadata.csv`} title='Descarregar dados'>âmbito geográfico dos contingentes e quanto às disposições relativas ao estacionamento dos táxis</a>. Os dados referem-se ao período entre {startYear} e {endYear} e foram recolhidos, pela AMT, junto dos 308 municípios portugueses. De forma a complementar os dados recolhidos e concretizar uma análise mais detalhada, foram também utilizados dados sobre a <a href={`${rawGitApi}@master/data/population.csv`} title='Descarregar dados'>população residente</a> e sobre as <a href={`${rawGitApi}@master/data/dormidas.csv`} title='Descarregar dados'>dormidas</a> em estabelecimentos hoteleiros, disponibilizados pelo <a href='http://www.ine.pt' title='Ir para INE' target='_blank' rel='noopener noreferrer'>Instituto Nacional de Estatística</a> na sua base de dados pública.</p>
          ...
          <div>
            ...
            <p>Existem dados relativos a {endYear} para todos os concelhos. Para os concelhos em que não existia informação disponível para todo o período compreendido entre {startYear} e {endYear}, os valores assumidos na análise resultam de imputação do primeiro valor disponível aos anos anteriores. Neste caso, a metodologia descrita é equivalente à utilização das taxas de variação anual apuradas para os concelhos com informação, na imputação dos valores dos concelhos sem dados para todos os anos.</p>
