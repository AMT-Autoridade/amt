...
var Concelho = React.createClass({
        ...
        <ul className='map-aa-list inline-list'>
          <li>
            <a
              ...
              href={`#/nuts/${this.props.nut.data.slug}${hash}`}
              title={`Ir para ${this.props.nut.data.name}`}
            >
              <span className='i-arrow-left2' />
              {this.props.concelho.data.name}
            </a>
          </li>
        </ul>
      </div>
  ...
  componentDidMount: function () {
    ...
    this.onWindowResize();
    this.props._fetchConcelho(
      this.props.params.nut,
      this.props.params.concelho
    );
  ...
  componentWillReceiveProps: function (nextProps) {
    if (
      this.props.params.concelho !== nextProps.params.concelho ||
      this.props.params.nut !== nextProps.params.nut
    ) {
      return this.props._fetchConcelho(
        nextProps.params.nut,
        nextProps.params.concelho
      );
    }
  },
  ...
  renderLicencas1000Chart: function (lic1000Data, id) {
    ...
    let tooltipFn = makeTooltip(entryIndex => {
      ...
      return (
        <ul>
          <li>
            <span className='tooltip-title'>{year}:</span>
          </li>
          {lic1000Data.datasets.map(o => (
            <li key={o.label}>
              <span
                style={{ backgroundColor: o.color }}
                className='tooltip-marker'
              />
              <span className='tooltip-label'>{o.label}:</span>{' '}
              <span className='tooltip-number'>
                {round(o.data[entryIndex])}
              </span>
            </li>
          ))}
          <span className='triangle' />
    ...
    let labels = lic1000Data.labels.map((o, i) =>
      i === 0 || i === l ? o : ''
    );
  ...
  renderTimelineChart: function () {
    ...
    let tooltipFn = makeTooltip(entryIndex => {
      ...
      return (
        <ul>
          <li>
            <span className='tooltip-title'>Licenças e Contingentes:</span>
          </li>
          <li>
            <span
              style={{ backgroundColor: '#1f8d8e' }}
              className='tooltip-marker'
            />
            <span className='tooltip-label'>Total Licenças:</span>
            <span className='tooltip-number'>
              {formatPT(year['lic-geral'] + year['lic-mob-reduzida'])}
            </span>
          </li>
          <li>
            <span className='tooltip-label'>Total Contingentes:</span>{' '}
            <span className='tooltip-number'>
              {formatPT(year['max-lic-geral'] + year['max-lic-mob-reduzida'])}
            </span>
          </li>
          <span className='triangle' />
        </ul>
    ...
    let labels = nationalTimeline.map((o, i) =>
      i === 0 || i === l ? o.year : ''
    );
    ...
    let chartData = {
      ...
      datasets: [
        {
          data: nationalTimeline.map(
            o => o['lic-geral'] + o['lic-mob-reduzida']
          ),
    ...
    return (
      <LineChart
        ...
        options={chartOptions}
        height={240}
        ref={this.addChartRef(`chart-timeline`)}
      />
    );
  },
  ...
  renderLic1000HabChart: function () {
    let chartLic1000Had = {
      ...
      datasets: _.sortBy(
        [
          {
            data: this.props.national.data.licencasTimeline.map(
              o => o['lic1000']
            ),
            label: 'Portugal',
            color: '#1f8d8e',
          ...
          {
            data: this.props.nut.data.data.licencasTimeline.map(
              o => o['lic1000']
            ),
          ...
          {
            data: this.props.concelho.data.data.licencasTimeline.map(
              o => o['lic1000']
            ),
  ...
  renderLic1000DormidasChart: function () {
    ...
    let chartLic1000Dormidas = {
      ...
      datasets: _.sortBy(
        [
          {
            data: this.props.national.data.dormidas
              .filter(o => _.includes(availableYears, o.year))
              .map(o => o.lic1000),
            label: 'Portugal',
            color: '#1f8d8e',
            backgroundColor: 'rgba(245, 245, 245, 0.2)'
          ...
          {
            data: this.props.concelho.data.data.dormidas
              .filter(o => _.includes(availableYears, o.year))
              .map(o => o.lic1000),
            label: this.props.concelho.data.name,
            color: '#256465',
  ...
  renderFileLink: function () {
    ...
    return (
      <div>
        {files.length ? (
          <p className='aa-doc-download'>
            <a href={files[0]} title='Descarregar documento'>
              Descarregue o documento relativo a {this.props.concelho.data.name}.
            </a>
          </p>
        ) : null}
      </div>
    ...
    return (
      <div id='page-content'>
        <div id='concelho' className='content-wrapper vertical-center'>
          <div className='map-wrapper'>{this.renderMap()}</div>

          <div className='section-wrapper'>
            <section className='section-container'>
              <header className='section-header'>
                <h3 className='section-category'>
                  <Link to={`/${hash}`} title='Ver Portugal'>
                    Portugal
                  </Link> &rsaquo; <Link
                    to={`/nuts/${nut.slug}${hash}`}
                    title={`Ver ${nut.name}`}
                  >
                    {nut.name}
                  </Link>
              ...
              <div className='section-content'>
                <div className='section-stats'>
                  <ul>
                    ...
                    <li>
                      ...
                      <span className='stat-description'>
                        Táxis licenciados por 1000 residentes.
                      </span>
                    </li>
                    <li>
                      <span className='stat-number'>{dormidas}</span>
                      <span className='stat-description'>
                        Táxis licenciados por 1000 dormidas.
                      </span>
                    </li>
                  </ul>
                </div>
                ...
                <ul className='table-distribution'>
                  <li className='table-header'>
                    <span className='table-cell table-scope'>
                      Âmbito Geográfico
                      <span className='block'>do Contingente</span>
                    </span>
                    <span className='table-cell table-parking'>
                      Regime(s) de <span className='block'>Estacionamento</span>
                    </span>
                    <span className='table-cell table-national'>
                      % do Total de <span className='block'>Licenças em Portugal</span>
                    </span>
                    <span className='table-cell table-residents'>
                  ...
                  <li>
                    <span className='table-cell table-scope'>
                      {contingente
                        ? this.contingenteMatrix[contingente]
                        : 'Não Definido'}
                    </span>
                    <div className='table-cell table-parking'>
                      <ul className='inline-list'>
                        <li
                          className={c('est est-livre', {
                            active: estacionamento.indexOf('livre') !== -1
                          })}
                        >
                          L
                        </li>
                        <li
                          className={c('est est-condicionado', {
                            active:
                              estacionamento.indexOf('condicionado') !== -1
                          })}
                        >
                          C
                        </li>
                        <li
                          className={c('est est-fixo', {
                            active: estacionamento.indexOf('fixo') !== -1
                          })}
                        >
                          F
                        </li>
                        <li
                          className={c('est est-escala', {
                            active: estacionamento.indexOf('escala') !== -1
                          })}
                        >
                          E
                        </li>
                ...
                <div className='graph-container'>
                  ...
                  <div className='graph'>
                    <h6 className='legend-title'>
                      Evolução dos táxis licenciados <span className='block'>por 1000 residentes</span>
                    </h6>
                    {this.renderLic1000HabChart()}
                  </div>
                  <div className='graph'>
                    <h6 className='legend-title'>
                      Evolução dos táxis licenciados <span className='block'>por 1000 dormidas</span>
                    </h6>
              ...
              <footer className='section-footer'>
                <p>
                  <strong>Nota I:</strong> Os valores dos indicadores devem ser analisados caso a caso e comparados com particular precaução. A consideração de outros fatores com influência na procura poderá melhor enquadrar as diferenças existentes.
                </p>
                <p>
                  <strong>Nota II:</strong> Dormidas nos estabelecimentos hoteleiros (estabelecimento cuja atividade principal consiste na prestação de serviços de alojamento e de outros serviços acessórios ou de apoio, mediante pagamento).
                </p>
              </footer>
            </section>
          </div>
